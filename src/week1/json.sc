...
object json {
  ...
  * 	case pat => x
  ...
  magical*/
  /*data base Query &
  traversal in one turn
  at once*/
  for {
    /*expression like 'x <- xs' is a generator
    'JObj( bindings )' pattern in a generator
    as implisit filter on 'data'*/
    JObj( bindings ) <- data
    /*? local 'val' using pattern matching ?
    bindings: Map[String, JSON]
    'key' == "phoneNumbers"
    JSeq   (elems: List[JSON])*/
